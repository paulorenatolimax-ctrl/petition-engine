...
## PRÉ-REQUISITO OBRIGATÓRIO: LEITURA DOS RAGs DE VACINAÇÃO

**ANTES de iniciar qualquer fase**, o agente DEVE ler todos os arquivos em:

```
/Users/paulo1844/Documents/_PROEX (A COMPLEMENTAR)/_(RAGs) - ARGUMENTAÇÃO (ESTUDO)_LINKS QUE REFORÇAM/2025/
```

Estes são RAGs de argumentação e estudo real — contêm:
- Padrões de argumentação que FUNCIONARAM em petições aprovadas
- Padrões de argumentação que FALHARAM e geraram RFEs/negações
- Links e referências que reforçam teses jurídicas
- Exemplos concretos de linguagem aceita pelo USCIS

**Como usar**:
1. Ler TODOS os arquivos da pasta (recursivamente, incluindo subpastas)
2. Extrair padrões POSITIVOS → usar como modelo de argumentação
3. Extrair padrões NEGATIVOS → usar como VACINA (evitar a todo custo)
4. Manter em contexto durante TODA a sessão de produção
5. Na Fase 3 (Produção), cross-check cada parágrafo contra os padrões negativos

**Esta leitura é AUTOMÁTICA e OBRIGATÓRIA — não depende de instrução do usuário.**
...
> O agente automaticamente lerá os RAGs de vacinação antes de iniciar a Fase 0.
> Não é necessário instruir manualmente.
