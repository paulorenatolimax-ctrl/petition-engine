# EB-1A Cover Letter Factory — v5.0 (Lições Vitória Carolina)

## Para subir no Projects do Claude.ai (tudo flat, sem subpastas)
### Atualizado em 18/03/2026 — Integração completa das lições do Caso Vitória Carolina
...
## CHANGELOG v4.0 → v5.0 (Caso Vitória Carolina — Março 2026)

### Arquivos ATUALIZADOS (4)

1. **FORBIDDEN_CONTENT.md**
   - Adicionada CATEGORIA 7: IDIOMA (Lição Vitória)
     - Regra absoluta: português brasileiro nativo em TODO o conteúdo narrativo
     - Exceções enumeradas: nomes de leis, casos judiciais, códigos O*NET, termos técnicos
     - Validação: rejeitar qualquer parágrafo inteiro em inglês
   - Adicionada CATEGORIA 8: PROPORÇÕES DO DOCUMENTO (Lição Vitória)
     - Introdução: 10-15%
     - Step 1: 40-50%
     - Step 2: 35-45% (regra crítica: se < 25% = documento incompleto)

2. **EVIDENCE_NAMING_CONVENTION.md**
   - DESCONTINUADO: sistema de sub-evidências com sufixo (XXa, XXb, XXc) do v3.0/v4.0
   - NOVA REGRA v5.0: CVs e documentos auxiliares são PARTE INTEGRANTE da evidência principal
   - Exemplo: Evidência 68 = Carta + CV (não "68a" nem "68.1")
   - Impacto: simplificação crítica da nomeação; Paulo determinou que sufixos não existem na prática

3. **QUALITY_GATES.md**
   - Adicionado GATE 7: PÓS-ASSEMBLY DOCX (Lição Vitória)
     - 7.1: Verificação de imagens (wp:anchor + wrapSquare, zero wp:inline)
     - 7.2: Correção de tabelas (tblInd=0, tblW=5000/pct, jc=center)
     - 7.3: Quebras de página (antes de CRITÉRIO, ETAPA 2, Partes B; zero no sumário)
     - 7.4: Tipografia (keepNext, widowControl, Garamond)
     - 7.5: Merge (XML manual, zero docxcompose; contagem de imagens)
     - 7.6: Notas de rodapé (manuais [1],[2]; manter, não converter a Word footnotes)
     - 7.7: Numeração (zero sufixos; CVs parte da evidência principal)
     - 7.8: Idioma (português brasileiro obrigatório)
     - 7.9: Validação O*NET (confirmado com résumé; 10+ ocorrências)

### Arquivos CRIADOS (1)

1. **DOCX_PRODUCTION_PIPELINE.md** (NOVO)
   - Guia técnico completo do pipeline de produção
   - 9 etapas: geração de thumbnails, inserção em .docx, conversão inline→anchor, merge, correções de tabela, quebras de página, layout, formatação, verificação final
   - Tabela de erros conhecidos + prevenção (10 erros críticos)
   - Procedimentos bash e Python para cada etapa

### Resumo das Mudanças Críticas

| Aspecto | v4.0 | v5.0 | Impacto |
|---------|------|------|--------|
| Idioma | Inglês permitido em corpo narrativo | Português brasileiro obrigatório | Retrabalho se violar |
| Sub-evidências | XXa, XXb, XXc (sufixo) | Descontinuado — parte da principal | Simplificação nomenclatura |
| Proporções | Sem regra explícita | 10-15% / 40-50% / 35-45% | Documento deve ser equilibrado |
| Imagens em tabelas | wp:inline (padrão python-docx) | wp:anchor + wrapSquare (mandatório) | Evita layout quebrado |
| Merge | docxcompose (perde imagens) | XML manual (mantém todas) | 100% de integridade |
| Page breaks | Sem critério claro | Apenas CRITÉRIO, ETAPA 2, Partes B | Sumário em páginas contínuas |
| Footnotes | Conversão a Word footnotes | Manuais [1],[2] (não converter) | Evita bugs de órfãs |
| O*NET validation | Sem gate específico | Gate 7.9: validação obrigatória | Evita código errado |

### Motivações (Lições Vitória)

1. **IDIOMA**: Claude Code produzia tudo em inglês → necessário rewrite 100%
2. **VOZ**: "the Beneficiary" → "I" criava artefatos ("I has") → português resolve
3. **EVIDÊNCIAS**: Sufixo 68.1, 69a não existem na prática → remover
4. **O*NET**: Código errado (29-1031 vs 11-9111) → validação gate
5. **PROPORÇÕES**: Step 2 só 9% (vs 45% alvo) → regra explícita
6. **THUMBNAILS**: python-docx usa wp:inline → quebra em tabelas → converter XML
7. **TABELAS**: Recuo esquerdo desaproveitava espaço → tblInd=0 obrigatório
8. **PAGE BREAKS**: Sem critério → sumário ficava fragmentado → regra clara
9. **LAYOUT**: Títulos órfãos e linhas viúvas → keepNext + widowControl
10. **FOOTNOTES**: Conversão a Word footnotes causava órfãs → manter manuais

---

*v5.0 — 18/03/2026 — Integração completa das lições do Caso Vitória Carolina.*
*Mudanças críticas: idioma, sub-evidências, proporções, pipeline DOCX.*
