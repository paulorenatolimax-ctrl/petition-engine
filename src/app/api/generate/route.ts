...
function buildAnteprojetoInstruction(client: any, system: any, docType: string, outputDir: string, rulesSection: string, selectedEndeavor?: string, selectedSocCode?: string): string {
  ...
  const lines = [
    ...
    '- 100% em PORTUGUES com ACENTUACAO CORRETA OBRIGATORIA. NUNCA escrever sem acentos (ex: "introducao" errado → "introdução" correto). Usar ção, ções, ão, ões, é, ê, á, í, ú, ã, õ em TODAS as palavras que exigem.',
  ...
  if (existingInstruction) {
    ...
    if (isPptx) {
      ...
      prompt = [
        ...
        '- ACENTUACAO PORTUGUESA OBRIGATORIA: Quando o documento for em portugues, usar TODOS os acentos corretamente (ção, ções, ão, ões, é, ê, á, í, ú, ã, õ, etc). NUNCA escrever "introducao" — escrever "introdução". NUNCA "sera" — escrever "será". NUNCA "area" — escrever "área". Isso vale para TODO texto em portugues no documento.',
